...
RSpec.describe HrrRbSsh::Message::Codable do
  ...
  context "when module has CONDITIONAL_DEFINITION that requires complementary message" do
    ...
    describe ".encode" do
      context "when arg does not contain an instance of Proc" do
        it "encodes #{{'SSH_MSG_MOCK' => 168, 'data' => 'testing', 'hidden data' => 'conditional'}.inspect} with complementary message #{{'require hidden' => true}.inspect} to \"A8 00 00 00 07 t e s t i n g 00 00 00 0B c o n d i t i o n a l\"" do
          expect( mixed_in.encode( {'SSH_MSG_MOCK' => 168, 'data' => 'testing', 'hidden data' => 'conditional'}, {'require hidden' => true} ) ).to eq( ["A8", "00000007", "testing", "0000000B", "conditional"].pack("H*H*a*H*a*") )
        end
      end

      context "when arg contains an instance of Proc" do
        it "encodes #{{'SSH_MSG_MOCK' => 168, 'data' => lambda { 'testing' }, 'hidden data' => lambda { 'conditional' }}.inspect} with complementary message #{{'require hidden' => lambda { true }}.inspect} to \"A8 00 00 00 07 t e s t i n g 00 00 00 0B c o n d i t i o n a l\"" do
          expect( mixed_in.encode( {'SSH_MSG_MOCK' => 168, 'data' => lambda { 'testing' }, 'hidden data' => lambda { 'conditional' }}, {'require hidden' => lambda { true }} ) ).to eq( ["A8", "00000007", "testing", "0000000B", "conditional"].pack("H*H*a*H*a*") )
        end
      end
    end

    describe ".decode" do
      it "decodes \"A8 00 00 00 07 t e s t i n g 00 00 00 0B c o n d i t i o n a l\" with complementary message #{{'require hidden' => true}.inspect} to #{{'SSH_MSG_MOCK' => 168, 'data' => 'testing', 'hidden data' => 'conditional'}.inspect}" do
        expect( mixed_in.decode( ["A8", "00000007", "testing", "0000000B", "conditional"].pack("H*H*a*H*a*"), {'require hidden' => true} ) ).to eq( {'SSH_MSG_MOCK' => 168, 'data' => 'testing', 'hidden data' => 'conditional'} )
      end
    end
  end
end
