...
RSpec.describe HrrRbSsh::SubclassWithPreferenceListable do
  let(:superclass){
    Class.new do
      @subclass_list = Array.new
      class << self
        include HrrRbSsh::SubclassWithPreferenceListable
      end
    end
  }
  ...
  let(:subclass0){
    Class.new(superclass) do
      ...
      const_set(:PREFERENCE, 0)
    end
  }
  let(:subclass1){
    Class.new(superclass) do
      const_set(:NAME, 'subclass1')
      const_set(:PREFERENCE, 1)
    end
  }
  let(:subclass2){
    Class.new(superclass) do
      const_set(:NAME, 'subclass2')
      const_set(:PREFERENCE, 2)
    end
  }
  ...
  context "superclass" do
    ...
    describe ".list_preferred" do
      context "when superclass does not have subclasses" do
        it "returns []" do
          expect(superclass.list_preferred).to eq []
        end
      end
      ...
      context "when superclass has subclasses" do
        ...
        it "returns an instance of Array that containes preferred subclasses' name" do
          expect(superclass.list_preferred).to include 'subclass1', 'subclass2'
        end
      end
    end
  end
  ...
  context "subclass" do
    ...
    describe ".list_preferred" do
      it "raises NoMethodError" do
        expect { subclass0.list_preferred }.to raise_error NoMethodError
      end
    end
  end
end
